...
**Предусловия:**  
- Установлены зависимости (make, компилятор RISC‑V, QEMU)
- Исходники проекта доступны локально

**Основной поток:**
1. Запустить `make qemu` в корне проекта.
2. Система по цепочке собирает стадии (stage1 → stage2), линкует и формирует артефакты ELF/BIN.
3. Автоматически стартует QEMU в режиме машины virt с консолью по UART; в терминале видны сообщения загрузки.
4. Экран консоли отображает сигнальные строки начальной инициализации (например, Stage1 OK, ENTER MAIN) для быстрой проверки.
5. Процесс завершается без ошибок, и разработчик подтверждает, что минимальная загрузка работает.

**Критерии приёмки:**  
- Команда завершается без ошибок  
- QEMU стартует  
- Появляется сообщение об успешном старте
...
**Основной поток:**
1. Выполнить `make clean`, затем `make all` и сохранить SHA256 всех ключевых артефактов.
2. Снова выполнить `make clean`, затем повторить `make all` и сохранить SHA256.
3. Сравнить убедиться, что хеши совпадают для каждого артефакта.
4. Зафиксировать результат: «воспроизводимо», если различий нет; иначе — «не воспроизводимо» с перечнем несовпадающих файлов.

**Критерии приёмки:**  
- SHA256 всех ключевых артефактов совпадает между двумя независимыми сборками
